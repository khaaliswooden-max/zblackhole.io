...
export default function WorldCanvas() {
  ...
  // Deep-clone on each mount so d3-force doesn't mutate module-level constants
  ...
  const getLinkColor = useCallback(
    (link: LinkObject) =>
      (link as unknown as GraphLink).type === 'CAUSED_BY'
        ? '#D85A30'
  ...
  const getLinkWidth = useCallback(
    (link: LinkObject) =>
      (link as unknown as GraphLink).type === 'CAUSED_BY' ? 1.5 : 1,
    []
  );
